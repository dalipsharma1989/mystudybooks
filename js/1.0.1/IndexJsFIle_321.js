...
function Load_TopSliderContent() {
    $.ajax({
        type: "POST",
        url: "Serv.asmx/WEB_load_sliders",
        data: '{ }',
...
function Load_TopBannerContent() {
    $.ajax({
        type: "POST",
        url: "Serv.asmx/WEB_load_banners",
        data: '{ }',
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        async: false,
        ...
        success: function (data) {
            ...
            for (var i = 0; i < TotLen; i++) {
                ...
                if (DataList[i].BannerID == "4") {
                    ...
                    var spancheck = "<span style='background:#ff4318;' class='new-item'>" + DataList[i].SpecialDiscount.toString().replace('.00', '') + "% OFF</span>";
...
function GetAllSliderNameInfo() {
    $.ajax({
        type: "POST",
        url: "Serv.asmx/WEB_load_ItemSliders",
        data: '{ }',
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        async: true,
...
function WEB_load_ItemSpecialOffer() {
    $.ajax({
        type: "POST",
        url: "Serv.asmx/WEB_load_ItemSpecialOffer",
        data: '{ }',
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        async: true,
        ...
        success: function (data) {
            ...
            if (DataList.length > 0) {
                for (var i = 0; i < TotLen; i++) {
                    ...
                    var HtmlRow = "<div class=''><a class='sliderImage-wrapper' href='view_book.aspx?productid=" + DataList[i].ProductID + "' title='" + DataList[i].BookName + "' id='SpecialOffer" + DataList[i].ProductID + "'>"
                        ...
                        + "<a class='product_tile'  href='view_book.aspx?productid=" + DataList[i].ProductID + "'>" + DataList[i].BookName + "</a>"
...
function Load_TodaySaying() {
    $.ajax({
        type: "POST",
        url: "Serv.asmx/WEB_Load_TodaySaying",
        data: '{ }',
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        async: true,
...
function LoadIndexCategoriesData() {
    $.ajax({
        type: "POST",
        url: "Serv.asmx/WEB_load_BookCategories",
        data: '{ }',
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        async: true,
        ...
        success: function (data) {
            ...
            for (var i = 0; i < TotLen; i++) {
                var HtmlRow = "<div class='col-lg-3 col-lg-6 col-sm-6 col-xs-12'><div class='small feature-box'>"
                    ...
                    + "<h6>" + DataList[i].BookCategoryDesc + "</h6><i class='fa fa-angle-right' aria-hidden='true'></i>"
                    + "</a></div></div>"
...
function WEB_load_IteminSliders(sliderName, SliderDivName) { 
    $.ajax({
        type: "POST",
        url: "Serv.asmx/WEB_load_ItemSliderDetail",
            ...
            if (DataList.length > 0) {
                for (var i = 0; i < TotLen; i++) {
                    ...
                    var HtmlRow = "<div class=''><a class='sliderImage-wrapper' href='view_book.aspx?productid=" + DataList[i].ProductId + "' title='" + DataList[i].BookName + "' id='SpecialOffer" + DataList[i].ProductId + "'>"
                        ...
                        + "<h4 style='text-align:center;'><div class='product-price d-flex' style='flex-direction:column;'><ul>" + ShownPrice + "</ul>"
                        + "<div class='add-links-wrap-cover'><div class='add-links-wrap'>"
                        + "<a id='ProductID_--" + DataList[i].ProductId + "' href='view_book.aspx?productid=" + DataList[i].ProductId + "' title='" + DataList[i].BookName + "' class='shopping-card'>"
                        + "<div class='show'><i class='fa fa-external-link'></i></div></a>"
                        + "<a href='#' id='itemID" + DataList[i].ProductId + "' title='Add To Cart'  onclick='return ModalEntry(\"" + DataList[i].ProductId + "\")'  class='" + addCartButtonShown + "'>"
                        + "<i class='fa fa-shopping-cart hidden-xs'></i><span class=''>Add to Cart</span></a>"
                        + "<a href='#' id=itemID" + DataList[i].ProductId + " title='Add To Wishlist'  onclick='return userwishlist(\"" + DataList[i].ProductId + "\")' >"
