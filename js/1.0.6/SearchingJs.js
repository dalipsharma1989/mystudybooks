...
function Load_AuthorList() {
    $.ajax({
        ...
        success: function (data) {
            ...
            var Count = 0;
            for (var i = 0; i < TotLen; i++) {
                var HtmlRow = "<div class='custom-control custom-checkbox'><input name='checkboxforAuthors' class='custom-control-input Author' type='checkbox' id='Author" + DataList[i].AuthorID
                            + "'><label id='Authorlbl" + DataList[i].AuthorID + "' class='custom-control-label' for='Author" + DataList[i].AuthorID + "'>" + DataList[i].AuthorName
                            + "&nbsp;<span class='text-muted'></span></label></div>";
